...
setPages.addEventListener('click',async(e)=>{
    if(e.target.classList.contains('setButton')){
        ...
        const getExpenceData = await axios.get(`http://184.73.124.55:3000/expence/getExpences?page=1&limit=${limit}`,{headers:{'Authorization':token}});
...
downloads.addEventListener('click',async()=>{
    try{
        const getIsPremium = await axios.get('http://184.73.124.55:3000/expence/isPremiumUser',{headers:{'Authorization':token}});
        if(getIsPremium.data.isPremiumUser){
            const docDownload = await axios.get('http://184.73.124.55:3000/expence/download',{headers:{'Authorization':token}});
...
leaderboard.addEventListener('click',async()=>{
    try{
        const result = await axios.get('http://184.73.124.55:3000/premium/getLeaderboard');
...
premium.onclick = async (e)=>{
    try{
        // console.log(token);
        const response = await axios.get('http://184.73.124.55:3000/purchase/premiumMembership',{headers:{'Authorization':token}});
        // console.log(response);
        const options ={
            'key':response.data.key_id,
            'order_id':response.data.order.id,
            'handler':async function (response){
                await axios.post('http://184.73.124.55:3000/purchase/updatePremiumMembership',{
...
addExpence.addEventListener('click',async(e)=>{
    ...
    try{
        ...
        const getPostExpence = await axios.post('http://184.73.124.55:3000/expence/postExpence',{amount , decription , category},{headers:{'Authorization':token}})
...
showExpences.addEventListener('click',async(e)=>{
    if(e.target.classList.contains('delete')){
        try{
            ...
            await axios.delete(`http://184.73.124.55:3000/expence/deleteExpence?id=${id}`,{headers:{'Authorization':token}});
...
window.addEventListener('DOMContentLoaded',async()=>{
    ...
    getExpenceData.data.allexpences.forEach(({id,amount,decription,category})=>createExpences(amount,decription,category,id))
    const getReports = await axios.get('http://184.73.124.55:3000/expence/reportUrl',{headers:{'Authorization':token}})
...
pageButton.addEventListener('click',async(e)=>{
    ...
    if(e.target.classList.contains('pages')){
        ...
        const limit = localStorage.getItem('limit') || 10;
        const getExpenceData = await axios.get(`http://184.73.124.55:3000/expence/getExpences?page=${page}&limit=${limit}`,{headers:{'Authorization':token}});
